...
class TestLinearSolver:
    ...
    def test_solve(entering_selection_rule):
        ...
        assert solver.solve() == pytest.approx(10.5)
        ...
        solver = LinearSolver(A, b, c, entering_selection_rule=entering_selection_rule)
        assert solver.solve() == pytest.approx(1827.0)
        ...
        c = np.array([2, 1])
        solver = LinearSolver(A, b, c, entering_selection_rule=entering_selection_rule)
        assert solver.solve() == pytest.approx(1.8571428571428572)
        ...
        c = np.array([3, 5])
        solver = LinearSolver(A, b, c, entering_selection_rule=entering_selection_rule)
        assert solver.solve() == pytest.approx(36.0)
